...
#cv_sets <- cached_creation("rda/cv_sets.rds",
#                           function() { create_cv_sets(train_set, K) })
#cv_sets <- create_cv_sets(train-set, K)
...
#rmses <- calc_kfold_rmses(cv_sets, lambdas, function(test_set, train_set) {
#  sapply(lambdas, function(lambda) {
#    model <- fit_model_reg_movie(train_set, lambda)
#    pred <- predict_reg_movie(test_set, model)
#    RMSE(test_set$rating, pred)
...
cv_sets <- create_cv_sets(edx, K)
rmses <- calc_kfold_rmses_reg_movie(cv_sets, lambdas)
best_lambda_reg_movie = lambdas[which.min(rmses)]

model <- fit_model_reg_movie(edx, lambda = best_lambda_reg_movie)
predicted_ratings <- predict_reg_movie(validation, model)

rmse_reg_movie_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                          tibble(method="Regularized Movie Effect Model", RMSE=rmse_reg_movie_effect))
rm(model, predicted_ratings)
```
...
rmses <- calc_kfold_rmses_reg_movie_user(cv_sets, lambdas)
best_lambda_reg_movie_user <- lambdas[which.min(rmses)]

model <- fit_model_reg_movie_user(edx, lambda = best_lambda_reg_movie_user)
predicted_ratings <- predict_reg_movie_user(validation, model)

rmse_reg_movie_user_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results, tibble(method="Regularized Movie + User Effect Model", RMSE=rmse_reg_movie_user_effect))
...
rmse_time_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
...
cache_edx_model_movie_time_effect_filename <- "rda/edx_model_movie_time_effect.rds"
if (use_cache & file.exists(cache_edx_model_movie_time_effect_filename)) {
  model_movie_time_effect <- readRDS(cache_edx_model_movie_time_effect_filename)
} else {
  model_movie_time_effect <- fit_model_movie_time_effect(edx)

  if (use_cache) {
    saveRDS(model_movie_time_effect, cache_edx_model_movie_time_effect_filename)
  }
}
predicted_ratings <- predict_movie_time_effect(validation, model_movie_time_effect)

rmse_movie_time_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                          tibble(method="Regularized Movie + User Effect + Movie Time Effect Model",
                                 RMSE=rmse_movie_time_effect))
rm(model_movie_time_effect, predicted_ratings)
...
cache_edx_model_genre_effect_filename <- "rda/edx_model_genre_effect.rds"
if (use_cache & file.exists(cache_edx_model_genre_effect_filename)) {
  model_genre_effect <- readRDS(cache_edx_model_genre_effect_filename)
} else {
  model_genre_effect <- fit_model_genre_reg_movie_user(edx, 5)

  if (use_cache) {
    saveRDS(model_genre_effect, cache_edx_model_genre_effect_filename)
  }
}
predicted_ratings <- predict_genre_reg_movie_user(validation, model_genre_effect, 5)

rmse_genre_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                          tibble(method="Regularized Movie + User Effect + Genre Effect Model",
                                 RMSE=rmse_genre_effect))
rm(model_genre_effect, predicted_ratings)
...
cache_edx_model_full_filename <- "rda/edx_model_full.rds"
if (use_cache & file.exists(cache_edx_model_full_filename)) {
  model_full <- readRDS(cache_edx_model_full_filename)
} else {
  model_full <- fit_model_full(edx, 5)

  if (use_cache) {
    saveRDS(model_full, cache_edx_model_full_filename)
  }
}
predicted_ratings <- predict_full(validation, model_full, 5)

rmse_full <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                          tibble(method="Regularized Movie + User Effect + Movie Time Effect + Genre Effect Model",
                                 RMSE=rmse_full))
rm(model_full, predicted_ratings)
...
The table below shows the results of training the various models against the full `edx` set, and then predicting against the `validation` set.

```{r final-results, include = TRUE}
kable(final_rmse_results, digits=5, caption="Validation RMSE Results")
```

* reg movie effect, best lambda is ???
