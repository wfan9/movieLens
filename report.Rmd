...
cache_cv_sets_filename <- "rda/cv_sets.rds"
if (use_cache & file.exists(cache_cv_sets_filename)) {
  cv_sets <- readRDS(cache_cv_sets_filename)
} else {
  cv_sets <- create_cv_sets(train_set, K)

  if (use_cache) {
    saveRDS(cv_sets, cache_cv_sets_filename)
  }
}
...
calc_kfold_rmses_reg_movie <- function(cv_sets, lambdas) {
...
cache_rmses_reg_movie_filename <- "rda/rmses_reg_movie.rds"
if (use_cache & file.exists(cache_rmses_reg_movie_filename)) {
  rmses <- readRDS(cache_rmses_reg_movie_filename)
} else {
  rmses <- calc_kfold_rmses_reg_movie(cv_sets, lambdas)

  if (use_cache) {
    saveRDS(rmses, cache_rmses_reg_movie_filename)
  }
}
best_lambda_reg_movie = lambdas[which.min(rmses)]

model <- fit_model_reg_movie(train_set, best_lambda_reg_movie)
...
```{r course-model-reg-movie-effects-lambda-plot, include = TRUE}
tibble(lambda = lambdas, rmse = rmses) %>%
  ggplot(aes(lambda, rmse)) +
  geom_point() +
  ggtitle("RMSE Versus Lambda For Regularized Movie Effect") +
  xlab("Lambda") +
  ylab("RMSE")
```

The optimal value for $\lambda$ is `r best_lambda_reg_movie`. Using that for training on the `train_set`, and evaluating against the `test_set`, the RMSE value obtained from this model is `r rmse_reg_movie_effect`.
...
#'
#' @return Mean RMSE for each lambda value.
calc_kfold_rmses_reg_movie_user <- function(cv_sets, lamdas) {
  calc_kfold_rmses(cv_sets, lambdas, function(test_set, train_set) {
    sapply(lambdas, function(lambda) {
...
cache_rmses_reg_movie_user_filename <- "rda/rmses_reg_movie_user.rds"
if (use_cache & file.exists(cache_rmses_reg_movie_user_filename)) {
  rmses <- readRDS(cache_rmses_reg_movie_user_filename)
} else {
  rmses <- calc_kfold_rmses_reg_movie_user(cv_sets, lambdas)

  if (use_cache) {
    saveRDS(rmses, cache_rmses_reg_movie_user_filename)
  }
}
...
rmse_results <- bind_rows(rmse_results,
                          tibble(method="Regularized Movie + User Effect Model",
                                 RMSE=rmse_reg_movie_user_effect))
...
```{r course-model-reg-movie-user-effects-lambda-plot, include = TRUE}
tibble(lambda = lambdas, rmse = rmses) %>%
  ggplot(aes(lambda, rmse)) +
  geom_point() +
  ggtitle("RMSE Versus Lambda For Regularized Movie And User Effect") +
  xlab("Lambda") +
  ylab("RMSE")
```
...
avg_rating_vs_date %>%
  ...
  ggtitle("Average Rating Versus Date") +
  xlab("Date") +
  ylab("Average Rating")

rm(avg_rating_vs_date)
```
...
resids %>%
  ...
  ggtitle("Residual Versus Date Closer Fit") +
  xlab("Date") +
  ylab("Residual")
...
fit_model_time_effect <- function(train_set, lambda, span = 0.10) {
  model <- fit_model_reg_movie_user(train_set, lambda)
...
model <- fit_model_time_effect(train_set, best_lambda_reg_movie_user)
predicted_ratings <- predict_time_effect(test_set, model)

rmse_time_effect <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                          tibble(method="Regularized Movie + User Effect + Week Time Effect Model",
                                 RMSE=rmse_time_effect))
rm(model, predicted_ratings)
```

Using loess for $f$, the resulting RMSE value is `r rmse_time_effect`, which is negligible improvement on the base regularized movie and user effect model with RMSE `r rmse_reg_movie_user_effect`.
...
small %>% group_by(movieId) %>%
  ...
  geom_point(alpha=0.01) +
  geom_smooth() +
  ggtitle("Good Movies Rating Versus Age") +
  xlab("Age (Weeks)") +
  ylab("Rating")
```
...
small %>% group_by(movieId) %>%
  ...
  geom_point(alpha=0.01) +
  geom_smooth() +
  ggtitle("Bad Movies Rating Versus Age") +
  xlab("Age (Weeks)") +
  ylab("Rating")
```
...
if (use_cache & file.exists(cache_model_movie_time_effect_filename)) {
  ...
  model_movie_time_effect <- fit_model_movie_time_effect(train_set, best_lambda_reg_movie_user)
...
rm(model_movie_time_effect, predicted_ratings)
```

The RMSE calculation obtained after building the model against the training set, and using it to predict against the test set is `r rmse_movie_time_effect`, which is an improvement over the base regulated movie and user effects model.
...
small %>% ggplot(aes(as.numeric(user_age, units="weeks"), rating)) +
  geom_point(alpha=0.01) +
  geom_smooth() +
  ggtitle("Rating Versus User Rating Age") +
  xlab("Age (Weeks)") +
  ylab("Rating")
```
...
edx %>% filter(genres == "(no genres listed)") %>%
  ...
  unique() %>%
  kable(caption = "Movies With No Genres")
```
...
split_movie_and_genres %>%
  ...
  ggtitle("Movie Genre Histogram") +
  xlab("Genre") +
  ylab("Count")

rm(movie_and_genres, split_movie_and_genres)
```
...
```{r ratings-vs-genres-plot, inclue = TRUE}
...
small %>%
  ...
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90)) +
  ggtitle("Rating By Movie Genre Boxplot") +
  xlab("Genre") +
  ylab("Rating")
```
...
user1_data_genre_bias <- user1_data %>%
...
user1_data_genre_bias %>% head(10) %>% kable(caption = "Distribution of Residual Per Genre")
...
```{r comedy-biases, include = TRUE}
user1_data_genre_bias %>% filter(genres == "Comedy") %>%
  kable(caption = "User 1 Comedy Bias")
user1_comedy_bias <- user1_data_genre_bias %>% filter(genres == "Comedy") %>% .$bias %>% mean()
rm(user1_data_genre_bias)
```

The above table shows the biases for the Comedy genre for the Comedy movies rated by user 1. User one's bias for the genre Comedy is then the average of these values (`r user1_comedy_bias`).
...
fit_model_genre_reg_movie_user <- function(train_set, num_chunks, lambda) {

  print("Fitting base model.")
  base_model <- fit_model_reg_movie_user(train_set, lambda)
...
if (use_cache & file.exists(cache_model_genre_effect_filename)) {
  ...
  model_genre_effect <- fit_model_genre_reg_movie_user(train_set, 5, best_lambda_reg_movie_user)
...
rm(model_genre_effect, predicted_ratings)
```
...
cache_model_full_filename <- "rda/model_full.rds"
if (use_cache & file.exists(cache_model_full_filename)) {
  model_full <- readRDS(cache_model_full_filename)
} else {
  model_full <- fit_model_full(train_set, 5, best_lambda_reg_movie_user)

  if (use_cache) {
    saveRDS(model_full, cache_model_full_filename)
  }
}
predicted_ratings <- predict_full(test_set, model_full, 5)

rmse_full <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
...
The RMSE for the full model is `r rmse_full`.
...
```{r final-training-validation-just-mean}
model <- fit_model_just_mean(edx)
predicted_ratings <- predict_just_mean(validation, model)
final_rmse_naive <- RMSE(validation$rating, predicted_ratings)

# Build up table of RMSE results.
final_rmse_results <- tibble(method = "Just the average", RMSE = final_rmse_naive)
rm(model, predicted_ratings)
```

```{r final-training-validation-movie-effects}
model <- fit_model_movie_effect(edx)
predicted_ratings <- predict_movie_effect(validation, model)

final_rmse_movie_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Movie effects", RMSE=final_rmse_movie_effect))
rm(model, predicted_ratings)
```

```{r final-training-validation-movie-user-effects}
model <- fit_model_movie_user_effect(edx)
predicted_ratings <- predict_movie_user_effect(validation, model)

final_rmse_movie_user_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Movie and User effects", RMSE=final_rmse_movie_user_effect))

rm(model, predicted_ratings)
```

```{r final-training-validation-regularized-movie-effects}

# Cross validation sets from edx set.
...
cache_final_rmses_reg_movie_filename <- "rda/final_rmses_reg_movie.rds"
if (use_cache & file.exists(cache_final_rmses_reg_movie_filename)) {
  final_rmses <- readRDS(cache_final_rmses_reg_movie_filename)
} else {
  final_rmses <- calc_kfold_rmses_reg_movie(final_cv_sets, lambdas)

  if (use_cache) {
    saveRDS(final_rmses, cache_final_rmses_reg_movie_filename)
  }
}
final_best_lambda_reg_movie = lambdas[which.min(final_rmses)]

model <- fit_model_reg_movie(edx, final_best_lambda_reg_movie)
predicted_ratings <- predict_reg_movie(validation, model)

final_rmse_reg_movie_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Regularized Movie Effect Model", RMSE=final_rmse_reg_movie_effect))
rm(model, predicted_ratings)
```

```{r final-training-validation-reg-movie-user-effects}
...
model <- fit_model_reg_movie_user(edx, final_best_lambda_reg_movie_user)
predicted_ratings <- predict_reg_movie_user(validation, model)

final_rmse_reg_movie_user_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Regularized Movie + User Effect Model",
                                       RMSE=final_rmse_reg_movie_user_effect))
...
```{r final-training-validation-avg-rating-vs-week}
model <- fit_model_time_effect(edx, final_best_lambda_reg_movie_user)
predicted_ratings <- predict_time_effect(validation, model)

final_rmse_time_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Regularized Movie + User Effect + Week Time Effect Model",
                                       RMSE=final_rmse_time_effect))
rm(model, predicted_ratings)
```

```{r final-training-validation-movie-time-effect}
cache_final_model_movie_time_effect_filename <- "rda/final_model_movie_time_effect.rds"
if (use_cache & file.exists(cache_final_model_movie_time_effect_filename)) {
  model_movie_time_effect <- readRDS(cache_final_model_movie_time_effect_filename)
} else {
  model_movie_time_effect <- fit_model_movie_time_effect(edx, final_best_lambda_reg_movie_user)

  if (use_cache) {
    saveRDS(model_movie_time_effect, cache_final_model_movie_time_effect_filename)
  }
}
predicted_ratings <- predict_movie_time_effect(validation, model_movie_time_effect)

final_rmse_movie_time_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Regularized Movie + User Effect + Movie Time Effect Model",
                                       RMSE=final_rmse_movie_time_effect))
rm(model_movie_time_effect, predicted_ratings)
```

```{r final-training-validation-genre-effect}
cache_final_model_genre_effect_filename <- "rda/final_model_genre_effect.rds"
if (use_cache & file.exists(cache_final_model_genre_effect_filename)) {
  model_genre_effect <- readRDS(cache_final_model_genre_effect_filename)
} else {
  model_genre_effect <- fit_model_genre_reg_movie_user(edx, 5, final_best_lambda_reg_movie_user)

  if (use_cache) {
    saveRDS(model_genre_effect, cache_final_model_genre_effect_filename)
  }
}
predicted_ratings <- predict_genre_reg_movie_user(validation, model_genre_effect, 5)

final_rmse_genre_effect <- RMSE(validation$rating, predicted_ratings)
final_rmse_results <- bind_rows(final_rmse_results,
                                tibble(method="Regularized Movie + User Effect + Genre Effect Model",
                                       RMSE=final_rmse_genre_effect))
rm(model_genre_effect, predicted_ratings)
```

```{r final-training-validation-full}
...
```{r final-results, include = TRUE}
kable(final_rmse_results, digits=5, caption="Validation RMSE Results")
```
...
Worst performance with a RMSE value of `r final_rmse_naive`. The average rating across all movies is `r mean(edx$rating)` which indicates a user bias towards viewing and rating movies that they will like.

### Movie Effect

RMSE of `r final_rmse_movie_effect` performs better than just the average of all movies. It represents the idea that some movies are widely regarded as "good", "bad" etc.

### Movie And User Effect

Improvement on the movie effect model with a RMSE of `r final_rmse_movie_user_effect`, representing the idea that users have different standards when rating. Some users may be very critical and consistently give low ratings, others may be the opposite.

### Regularized Movie Effect Model.

The regularization penalty term was calculated using k-fold cross validation with a value of `r final_best_lambda_reg_movie`, which lead to only a slight improvement over the non-regularized version, `r final_rmse_reg_movie_effect` versus `r final_rmse_movie_effect`.

### Regularized Movie and User Effects Model

Using cross validation, the best $\lambda$ value was `r final_best_lambda_reg_movie_user`. Regularization has somewhat of an improvement over the non-regularized version, `r final_rmse_reg_movie_user_effect` versus  `r final_rmse_movie_user_effect`. But it is not as large of an improvement obtained by adding another bias term to the model.
...
### Movie Time Effect Model.

A term representing the movie effect over time was added, by fitting loess models per movie. An improved RMSE value of `r final_rmse_movie_time_effect` was obtained compared to the base model value of `r final_rmse_reg_movie_user_effect`. Generally, "good" movies will receive positive ratings long after they are first released as users will be attracted to watching older "good" movies, while "bad" movies will not attract any ratings the older they are.

### Genres Model

The bias of individual users towards different genres was modeled, and the resulting RMSE of `r final_rmse_genre_effect` was a significant improvement over the base model value of `r final_rmse_reg_movie_user_effect`. Movie genre has strong predictive power on an individual user basis.

### Full Model

The full model (base model with genre and movie time effects) gave the best RMSE value of `r final_rmse_full`. The largest components contributing to this are the regularized movie and user effects. These present an improvement of approximately `r abs(round(final_rmse_reg_movie_user_effect - final_rmse_naive, 3))` over just taking the average rating. The user genre effect is the next largest contributing roughly `r abs(round(final_rmse_genre_effect - final_rmse_reg_movie_user_effect, 3))` improvement, followed by the movie time effect `r abs(round(final_rmse_movie_time_effect - final_rmse_reg_movie_user_effect, 3))`.
...
One term represents the effect of time on movie bias, and was modeled using loess. The other term was for representing individual user biases towards movie genres. A final RMSE value of `r final_rmse_full` was obtained by training on the `edx` set and evaluating against the `validation` set.
