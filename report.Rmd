...
fit_model_movie_user_effect <- function(train_set) {
  base_model <- fit_model_movie_effect(tain-set)
  b_u <- train_set %>%
...
The movie effect is regularized by providing a term that penalizes movies with few ratings.
...
```
The plot shows that there is slight variation of the average rating over time, so there may be an increase in prediction accuracy if we add another term to the prediction model that accounts for this. The formula is (based on the course exercise) $Y_{u,i} = \mu + b_{i}(\lambda) + b_{u}(\lambda) + f(d_{u,i}) + \varepsilon_{u,i}$ where $d_{u,i}$ is the week that user $u$ rated movie $i$ and $f$ is a smooth function.
...
predicted_ratings <- predict_movie_time_effect(model_movie_time_effect, test_set)
...
predicted_ratings <- predict_reg_movie_user(model, user1_data)
...
$w_{i,u,k} = n_{u,k} / \sum_{k=1}^K x_{i,k}n_{u,k}$ ($x_{i,k}$ defined as previously).

The residual for a movie $i$, rated by user $u$ can be expressed as:
...
#' Calculate individual user's bias for genres.
#'
#' @param data Movie data.
#' @param user ID of a user within the data
...
#'
#calc_user_genre_bias <- function(data, user, base_model, base_predict_func) {
#  # Filter for only necessary data and then calculate residuals.
#  user_data <- data %>% filter(userId == user)
#  resids <- user_data$rating - base_predict_func(base_model, user_data)

#  # Split the genres into separate rows, then calculate counts of how often each genre occurs.
#  # Then, per movie, apportion the residual for that movie between the genres of the movie,
#  # using a weight dependent on the ratios of the genre counts i.e. if a user watches a certain
#  # genre more than another, then it will receive a larger portion of the residual.
#  user_data <- user_data %>%
...
#' @param base_model Base model used to calculate predictions.
#' @param base_predict_func Function that takes the base_model and movie data as input, then calculates predictions to then determine residuals.
#
...
calc_user_genre_bias_chunked <- function(data, base_model, base_predict_func, num_chunks = 1) {
  ...
  cat("@@@@ num_chunks ", num_chunks)
  #chunks <- ifelse(num_chunks == 1,
  #                 list(user_ids),
  #                 split(user_ids, cut(seq_along(user_ids), num_chunks, labels = FALSE)))
  ...
  cat("@@@@ num_chunks ", num_chunks, " ", length(chunks))
  cat("chunk:")
  i <- 0
...
#' Calculate multiple users' biases for genres.
#'
#' @param data Movie data.
#' @param users Vector of user IDs.
#' @param base_model Base model used to calculate predictions, from which residuals are calculated.
#' @param base_predict_func Function that takes the base_model and movie data as input, then calculates predictions.
#'
#batch_calc_user_genre_bias <- function(data, users, base_model, base_predict_func) {
#  progress_len = 100
...
#    }
#    calc_user_genre_bias(data, user, base_model, base_predict_func)
#  })
#  bind_rows(results)
#}
...
fit_model_genre_reg_movie_user <- function(train_set, num_chunks = 1) {
  ...
  #@@@@user_genre_bias <- batch_calc_user_genre_bias(train_set, user_ids, base_model, predict_reg_movie_user)
...
#' @param test_set Movie data to predict against.
#' @param num_chunks To reduce memory requirements, perform calculations on chunks of data. A value of 1 indictes no split.
predict_genre_reg_movie_user <- function(model, test_set, num_chunks = 1) {
  
  print("Predicting base model values.")
  preds <- predict_reg_movie_user(model$base_model, test_set)
...
Sys.time()
predicted_ratings <- predict_genre_reg_movie_user(model_genre_effect, test_set, 5)
Sys.time()

rmse_genre_effect <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results, tibble(method="Regularized Movie + User Effect + Genre Effect Model", RMSE=rmse_genre_effect))

#rm(movie_genre_effect)
```


```{r just-avg-movie-model}
mu <- mean(edx$rating)
