...
calc_user_genre_bias_chunked <- function(data, base_model, base_predict_func, num_chunks = 1) {
  ...
  chunks <- user_ids %>% group_by((row_number() - 1) %/% (n() / num_chunks)) %>% nest %>% pull(data)

  cat("chunk:")
  i <- 0
  genre_biases <- lapply(chunks, function(data) {
    i <<- i + 1
    cat(" ", i)
    ...
    data %>% select(userId, movieId, genres) %>%
      separate_rows(genres, sep = "\\|") %>%
      left_join(model$user_genre_bias, by = c("userId", "genres")) %>%
      group_by(userId, movieId) %>%
      summarise(bias = sum(bias))
  })
...
#'
fit_model_genre_reg_movie_user <- function(train_set) {
...
* Need to save and load objects from local file. have var to turn on/off. Load if the file exists, otherwise create, then save if caching on.
