...
calc_kfold_rmses_reg_movie <- function(cv_sets, lamdas) {
...
model <- fit_model_reg_movie(train_set, lambda = best_lambda_reg_movie)
...
#' @return Model consisting of value "base_model" (regularized movie and user effect model), and
#'  frame "user_genre_bias".
fit_model_full <- function(train_set, num_chunks = 1) {
...
if (use_cache & file.exists(cache_model_full_filename)) {
  ...
  model_full <- fit_model_full(train_set, 5)
...
model <- fit_model_reg_movie(edx, lambda = final_best_lambda_reg_movie)
...
model <- fit_model_reg_movie_user(edx, lambda = final_best_lambda_reg_movie_user)
...
model <- fit_model_time_effect(edx)
...
if (use_cache & file.exists(cache_final_model_movie_time_effect_filename)) {
  ...
  model_movie_time_effect <- fit_model_movie_time_effect(edx)
...
if (use_cache & file.exists(cache_final_model_genre_effect_filename)) {
  ...
  model_genre_effect <- fit_model_genre_reg_movie_user(edx, 5)
...
if (use_cache & file.exists(cache_final_model_full_filename)) {
  ...
  model_full <- fit_model_full(edx, 5)
